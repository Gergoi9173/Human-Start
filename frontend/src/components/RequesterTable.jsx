...
export default function RequesterTable({ requesters, resources, allocations, onAddAllocation, onDeleteAllocation, onUpdateAllocation }) {
  const [expanded, setExpanded] = useState({});

  const toggleExpand = (requesterId) => {
    setExpanded(prev => ({ ...prev, [requesterId]: !prev[requesterId] }));
  };

  const getAllocationsForRequester = (requesterId) => {
    return allocations.filter(a => a.requester_id === requesterId);
  };

  const getTotalPercentage = (reqAllocations) => {
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left">
          <thead className="text-xs text-muted-foreground uppercase bg-muted/50">
            <tr>
              <th scope="col" className="px-6 py-4 font-medium w-1/3">Requester</th>
          ...
          <tbody className="divide-y divide-border">
            {requesters.map((requester) => {
              const reqAllocations = getAllocationsForRequester(requester.id);
              const total = getTotalPercentage(reqAllocations);
              const isExpanded = expanded[requester.id];

              return (
                <Fragment key={requester.id}>
                  <tr className={`hover:bg-muted/30 transition-colors cursor-pointer ${isExpanded ? 'bg-muted/10' : ''}`} onClick={() => toggleExpand(requester.id)}>
                    <td className="px-6 py-4 font-medium text-foreground">
                      {requester.name}
                    </td>
                    <td className="px-6 py-4 text-muted-foreground text-xs">
                      {reqAllocations.length} resource{reqAllocations.length !== 1 ? 's' : ''}
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end space-x-2">
                        {/* We don't necessarily have a single resource to add to here, so we might just open the form without a pre-selected resource */}
                        <button
